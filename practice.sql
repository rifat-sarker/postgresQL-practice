...
-- 01. show employees and departments data
SELECT * from employees
JOIN departments on employees.department_id = departments.department_id;


-- USING -- same output as join -- when we have common field in two columns we can use USING method
SELECT * from employees
JOIN departments USING(department_id);



-- 02. show department name with avg salary -- round is used to nearest intger numbers
...
-- 03. count employees in each department
...
-- 04 . find the department name with the highest avg salary
SELECT department_name, round(avg(salary)) as avg_salary from employees
    JOIN departments USING(department_id)
    GROUP BY department_name
...
-- 06. find customers who have placed more than two orders and calculate the total amount spent by each of those customers 
SELECT customer_id, count(order_id), sum(total_amount) as total_amount from orders GROUP BY customer_id HAVING count(order_id) > 2 ;
